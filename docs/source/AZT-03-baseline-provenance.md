...
- Current production target remains `lstamduchn.vn`. No `v1.3.16` production deployment is claimed. The prior owner-approved `v1.3.15` preflight was **BLOCKED_EXTERNAL_ACCESS** before mutation because GitHub-hosted runner HTTPS stalled after ClientHello across all observed IPv4 backends and WPVibe fallback was rate-limited. Fresh live Theme version remains UNKNOWN and `1.3.10` remains only the last verified state until a new secure authenticated preflight succeeds. Historical blocker evidence: `docs/evidence/V1_3_15_LSTAMDUCHN_DEPLOYMENT_PREFLIGHT_BLOCKED_20260920.md`.
