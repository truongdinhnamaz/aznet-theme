...
**NEXT — PR #146 is the AZnet Theme 1.3.7 client-delivery candidate after client visual QA clarified the Law 01 layout contract: section surfaces may be full-width, but inner objects must align to the shared content shell. Production head `e0e2cd9376ab81dbcea4853581627e48361b76bd` passes the new static and browser geometry regression and produced the deterministic 1.3.7 package. Final docs-head CI must be green before canonical merge is considered. Git tag/GitHub Release and production deployment remain separate gates; RootProfile-backed Team integration remains `BLOCKED_EXTERNAL_CONTRACT` at issue #112.**

### AZnet Theme 1.3.7 client-delivery candidate — 18/09/2026

Client visual QA first led PR #145 to widen the inner Hero grid, but the subsequent screenshot clarified the actual design rule: **full-width applies to the section surface, not to bypassing the common inner shell**. The Theme's existing base contract already states that Law 01 section surfaces may be full width while inner content remains constrained by `--aznet-theme-container-shell`.

PR #146 adds a regression that compares Hero inner-grid geometry against Trust and Services at desktop widths, including 1920px. RED head `d0a3b87144d7f4f09342d38d256162debed29b5f` failed because the Burgundy variant still overrode the shared container with `width: 100%; max-width: none`. Minimal GREEN head `e0e2cd9376ab81dbcea4853581627e48361b76bd` removes that override only; the section remains viewport-wide and the approved 48/52 Hero split is preserved.

The 1.3.7 release identity remains correct because these production bytes post-date the 1.3.6 package. Y5 on `e0e2cd9...` produced deterministic `aznet-theme-1.3.7.zip`, 145 production files, 108/108 packaged PHP lint PASS, SHA-256 `aa1f2a4798b81c7b579c8a1b2a03adaa1932837fd3c277a0b7cbb72d5fe23e4a`. Run `35358986591`; promoted-package artifact `10553996265`, digest `sha256:328948807a5e40a6ece314ca1eb78965c0a3f519839935ff8fb8bf8ab0141bc0`.
