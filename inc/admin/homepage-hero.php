...
/** Stable content hash used to reject stale form submissions. */
function homepage_hero_form_content_hash( string $content ): string {
    return hash( 'sha256', trim( $content ) );
}

/** Exact hash of the pre-form D-030 default scaffold. */
function homepage_hero_legacy_scaffold_hash(): string {
    return '0ece22abcd0b9b93109000b43e36a96ce830236e2005b53327467a6867694a10';
}

function homepage_hero_form_is_legacy_scaffold_content( string $content ): bool {
    return hash_equals( homepage_hero_legacy_scaffold_hash(), homepage_hero_form_content_hash( $content ) );
}

/** Return the machine role stored in universal block metadata. */
...
/** @param array<int,array<string,mixed>> $blocks @param array<string,array<int,array<string,mixed>>> $found */
...
function homepage_hero_form_media_id_from_block( array $block ): int {
    foreach ( (array) ( $block['innerBlocks'] ?? [] ) as $child ) {
        if ( is_array( $child ) && 'core/image' === (string) ( $child['blockName'] ?? '' ) ) {
            $id = isset( $child['attrs']['id'] ) ? (int) $child['attrs']['id'] : 0;
            return $id > 0 ? $id : 0;
        }
    }
    return 0;
...
/** Parse only explicitly role-marked Core blocks; no title/position/content guessing. */
function homepage_hero_form_model_from_content( string $content ): ?array {
    $blocks = parse_blocks( $content );
    if ( ! is_array( $blocks ) || [] === $blocks ) { return null; }
    $found = [];
    homepage_hero_form_collect_role_blocks( $blocks, $found );
    $roles = homepage_hero_form_roles();
    foreach ( $roles as $role ) {
        if ( ! isset( $found[ $role ] ) || 1 !== count( $found[ $role ] ) ) { return null; }
    }
    return [
...
function homepage_hero_form_is_managed_content( string $content ): bool {
    return null !== homepage_hero_form_model_from_content( $content );
}
...
function homepage_hero_form_default_model(): array {
    return [
        'eyebrow' => __( 'Thông điệp mở đầu', 'aznet-theme' ),
        'title' => __( 'Viết tiêu đề Hero của bạn tại đây', 'aznet-theme' ),
        'value' => __( 'Viết dòng giá trị chính của Hero.', 'aznet-theme' ),
        'lead' => __( 'Viết thông điệp hoặc khẩu hiệu hỗ trợ.', 'aznet-theme' ),
        'primary_label' => __( 'Liên hệ tư vấn', 'aznet-theme' ),
        'primary_url' => '',
        'secondary_label' => __( 'Xem dịch vụ', 'aznet-theme' ),
        'secondary_url' => '',
        'image_id' => 0,
        'trust_1' => __( 'Tư vấn rõ ràng', 'aznet-theme' ),
        'trust_2' => __( 'Giải pháp thực tiễn', 'aznet-theme' ),
        'trust_3' => __( 'Bảo mật thông tin', 'aznet-theme' ),
        'trust_4' => __( 'Đồng hành tận tâm', 'aznet-theme' ),
    ];
}

function homepage_hero_form_text_block_html( array $block, string $text ): string {
    $name = (string) ( $block['blockName'] ?? '' );
    $class = isset( $block['attrs']['className'] ) && is_string( $block['attrs']['className'] ) ? trim( $block['attrs']['className'] ) : '';
    if ( 'core/heading' === $name ) {
        $level = isset( $block['attrs']['level'] ) ? max( 1, min( 6, (int) $block['attrs']['level'] ) ) : 2;
        $classes = trim( 'wp-block-heading ' . $class );
        return '<h' . $level . ( '' !== $classes ? ' class="' . esc_attr( $classes ) . '"' : '' ) . '>' . esc_html( $text ) . '</h' . $level . '>';
    }
    $classes = $class;
    return '<p' . ( '' !== $classes ? ' class="' . esc_attr( $classes ) . '"' : '' ) . '>' . esc_html( $text ) . '</p>';
}

function homepage_hero_form_button_html( array $block, string $label, string $url ): string {
    $class = isset( $block['attrs']['className'] ) && is_string( $block['attrs']['className'] ) ? trim( $block['attrs']['className'] ) : '';
    $outer = trim( 'wp-block-button ' . $class );
    ...
    if ( '' !== $background ) { $link .= ' has-' . $background . '-background-color has-background'; }
...
function homepage_hero_form_media_children( int $image_id ): array {
    if ( $image_id > 0 ) {
        ...
        if ( is_string( $url ) && '' !== $url ) {
            ...
            $html = '<figure class="wp-block-image size-large"><img src="' . esc_url( $url ) . '" alt="' . esc_attr( $alt ) . '" class="wp-image-' . esc_attr( (string) $image_id ) . '"/></figure>';
            return [[
                'blockName' => 'core/image',
                'attrs' => [ 'id' => $image_id, 'sizeSlug' => 'large', 'linkDestination' => 'none' ],
                'innerBlocks' => [],
                'innerHTML' => $html,
                'innerContent' => [ $html ],
            ]];
    ...
    return [[
        ...
        'attrs' => [ 'align' => 'center', 'className' => 'aznet-theme-homepage-hero-content__media-help', 'fontSize' => 'sm' ],
        'innerBlocks' => [],
        'innerHTML' => $html,
        'innerContent' => [ $html ],
    ]];
...
/** Update only role-marked blocks and preserve all unrelated/advanced blocks. */
function homepage_hero_form_update_blocks( array $blocks, array $model ): array {
    $roles = homepage_hero_form_roles();
    $textMap = [
        $roles['eyebrow'] => 'eyebrow', $roles['title'] => 'title', $roles['value'] => 'value', $roles['lead'] => 'lead',
        $roles['trust_1'] => 'trust_1', $roles['trust_2'] => 'trust_2', $roles['trust_3'] => 'trust_3', $roles['trust_4'] => 'trust_4',
    ];
    foreach ( $blocks as &$block ) {
        if ( ! is_array( $block ) ) { continue; }
        $role = homepage_hero_form_block_role( $block );
        if ( isset( $textMap[ $role ] ) ) {
            $html = homepage_hero_form_text_block_html( $block, (string) ( $model[ $textMap[ $role ] ] ?? '' ) );
            $block['innerHTML'] = $html; $block['innerContent'] = [ $html ];
            continue;
        }
        if ( $roles['primary_cta'] === $role || $roles['secondary_cta'] === $role ) {
            $prefix = $roles['primary_cta'] === $role ? 'primary' : 'secondary';
            $url = (string) ( $model[ $prefix . '_url' ] ?? '' );
            $label = (string) ( $model[ $prefix . '_label' ] ?? '' );
            if ( '' !== $url ) { $block['attrs']['url'] = $url; } else { unset( $block['attrs']['url'] ); }
            $html = homepage_hero_form_button_html( $block, $label, $url );
            $block['innerHTML'] = $html; $block['innerContent'] = [ $html ];
            continue;
        }
        if ( $roles['media'] === $role && 'core/column' === (string) ( $block['blockName'] ?? '' ) ) {
            $children = homepage_hero_form_media_children( (int) ( $model['image_id'] ?? 0 ) );
            $classes = 'wp-block-column';
            if ( isset( $block['attrs']['verticalAlignment'] ) && is_string( $block['attrs']['verticalAlignment'] ) && '' !== $block['attrs']['verticalAlignment'] ) { $classes .= ' is-vertically-aligned-' . sanitize_html_class( $block['attrs']['verticalAlignment'] ); }
            if ( isset( $block['attrs']['className'] ) && is_string( $block['attrs']['className'] ) && '' !== trim( $block['attrs']['className'] ) ) { $classes .= ' ' . trim( $block['attrs']['className'] ); }
            $open = '<div class="' . esc_attr( $classes ) . '">'; $close = '</div>';
            $block['innerBlocks'] = $children;
            $block['innerContent'] = [ $open, null, $close ];
            $block['innerHTML'] = $open . (string) ( $children[0]['innerHTML'] ?? '' ) . $close;
            continue;
        }
        if ( isset( $block['innerBlocks'] ) && is_array( $block['innerBlocks'] ) && [] !== $block['innerBlocks'] ) {
            $block['innerBlocks'] = homepage_hero_form_update_blocks( $block['innerBlocks'], $model );
        }
    }
    unset( $block );
    return $blocks;
...
function homepage_hero_form_managed_content( array $model ): string {
    $scaffold = homepage_hero_scaffold_content();
    if ( '' === $scaffold ) { return ''; }
    $blocks = parse_blocks( $scaffold );
    return serialize_blocks( homepage_hero_form_update_blocks( $blocks, $model ) );
}

function homepage_hero_form_updated_content( string $content, array $model ): string {
    if ( ! homepage_hero_form_is_managed_content( $content ) ) { return ''; }
    return serialize_blocks( homepage_hero_form_update_blocks( parse_blocks( $content ), $model ) );
}
...
function homepage_hero_form_post_model(): array {
    ...
    return [
        'eyebrow' => $text( 'homepage_hero_eyebrow' ),
        'title' => $text( 'homepage_hero_title' ),
        'value' => $text( 'homepage_hero_value' ),
        'lead' => $textarea( 'homepage_hero_lead' ),
        'primary_label' => $text( 'homepage_hero_primary_label' ),
        'primary_url' => $url( 'homepage_hero_primary_url' ),
        'secondary_label' => $text( 'homepage_hero_secondary_label' ),
        'secondary_url' => $url( 'homepage_hero_secondary_url' ),
        'image_id' => isset( $_POST['homepage_featured_image_id'] ) ? absint( $_POST['homepage_featured_image_id'] ) : 0,
        'trust_1' => $text( 'homepage_hero_trust_1' ),
        'trust_2' => $text( 'homepage_hero_trust_2' ),
        'trust_3' => $text( 'homepage_hero_trust_3' ),
        'trust_4' => $text( 'homepage_hero_trust_4' ),
    ];
}

function homepage_hero_form_require_source(): \WP_Post {
    require_law01_hero_scope();
    $source_id = isset( $_POST['homepage_hero_source_id'] ) ? absint( $_POST['homepage_hero_source_id'] ) : 0;
    $mapped_id = (int) homepage_source_value( 'law-01', 'hero', settings() );
    if ( $source_id <= 0 || $source_id !== $mapped_id ) { wp_die( esc_html__( 'Nguồn Hero không còn khớp với cấu hình Luật 01.', 'aznet-theme' ) ); }
    $hero = homepage_hero_candidate_reference( $source_id );
    if ( ! $hero instanceof \WP_Post || ! current_user_can( 'edit_post', $source_id ) ) { wp_die( esc_html__( 'Hero WordPress không hợp lệ hoặc bạn không có quyền chỉnh sửa.', 'aznet-theme' ) ); }
    return $hero;
}

/** Save the bounded simple form into the existing WordPress-owned synced block. */
function handle_homepage_hero_form_save(): void {
    ...
    check_admin_referer( 'aznet_theme_save_homepage_hero_form' );
    $hero = homepage_hero_form_require_source();
    ...
    $model = homepage_hero_form_post_model();
    if ( (int) $model['image_id'] > 0 && ! wp_attachment_is_image( (int) $model['image_id'] ) ) { wp_die( esc_html__( 'Ảnh Hero không hợp lệ.', 'aznet-theme' ) ); }
    $updated_content = homepage_hero_form_updated_content( (string) $hero->post_content, $model );
    if ( '' === $updated_content ) { wp_die( esc_html__( 'Không thể cập nhật Hero bằng form đơn giản.', 'aznet-theme' ) ); }
    $update = [ 'ID' => $hero->ID, 'post_content' => $updated_content ];
    if ( 'draft' === $hero->post_status ) {
        if ( ! current_user_can( 'publish_posts' ) ) { wp_die( esc_html__( 'Bạn không có quyền xuất bản Hero lên website.', 'aznet-theme' ) ); }
        $update['post_status'] = 'publish';
    }
    $result = wp_update_post( $update, true );
    if ( is_wp_error( $result ) ) { wp_die( esc_html( $result->get_error_message() ) ); }
    $variants = homepage_hero_library_variants();
    $requested = isset( $_POST['homepage_hero_variant'] ) ? sanitize_key( wp_unslash( $_POST['homepage_hero_variant'] ) ) : 'split';
    $theme_settings = settings();
...
function handle_homepage_hero_form_upgrade(): void {
    if ( ! current_user_can( 'edit_theme_options' ) ) { wp_die( esc_html__( 'Bạn không có quyền thay đổi Hero trang chủ.', 'aznet-theme' ) ); }
    check_admin_referer( 'aznet_theme_upgrade_homepage_hero_form' );
    $hero = homepage_hero_form_require_source();
    ...
    $content = homepage_hero_form_managed_content( homepage_hero_form_default_model() );
