...
<div class="aznet-theme-law01-container aznet-theme-law01-profile__grid">
    ...
    <div class="aznet-theme-law01-profile__team">
        ...
        <?php if ( [] !== $members ) : ?><div class="aznet-theme-law01-profile__members">
            <?php foreach ( $members as $member ) : if ( ! $member instanceof \WP_Post ) { continue; } $member_image = has_post_thumbnail( $member ) ? get_the_post_thumbnail( $member, 'medium_large', [ 'class' => 'aznet-theme-law01-profile__member-image' ] ) : ''; $member_summary = trim( (string) get_the_excerpt( $member ) ); ?>
            <article class="aznet-theme-law01-profile__member">
                <?php if ( '' !== $member_image ) : ?><a href="<?php echo esc_url( get_permalink( $member ) ); ?>"><?php echo wp_kses_post( $member_image ); ?></a><?php endif; ?>
                <h3><a href="<?php echo esc_url( get_permalink( $member ) ); ?>"><?php echo esc_html( get_the_title( $member ) ); ?></a></h3>
                <?php if ( '' !== $member_summary ) : ?><p><?php echo esc_html( $member_summary ); ?></p><?php endif; ?>
            </article>
            <?php endforeach; ?>
        </div><?php endif; ?>
        <p><a class="aznet-theme-law01-button aznet-theme-law01-button--secondary" href="<?php echo esc_url( get_permalink( $team ) ); ?>"><?php esc_html_e( 'Xem thêm về đội ngũ', 'aznet-theme' ); ?> <span aria-hidden="true">→</span></a></p>
    </div>
    <?php endif; ?>
</div>
</section>
