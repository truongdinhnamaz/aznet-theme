...
try{const body=await fs.readFile('ops/about-preview/about-body.css','utf8');css=baseline.split(marker)[0]+body;mode='candidate-body-replacement';}catch(e){if(e.code!=='ENOENT')throw e;}
...
for(const width of [390,768,1024,1440,1920]){
 ...
 const metrics=await page.evaluate(()=>{
  ...
  return{viewport:document.documentElement.clientWidth,scroll:document.documentElement.scrollWidth,sections:sections.map(el=>({cls:el.className,box:box(el),pad:parseFloat(getComputedStyle(el).paddingLeft),bg:getComputedStyle(el).backgroundColor,image:getComputedStyle(el).backgroundImage,radius:getComputedStyle(el).borderRadius,h2:el.querySelector('h2')?{...box(el.querySelector('h2')),font:getComputedStyle(el.querySelector('h2')).fontFamily,size:parseFloat(getComputedStyle(el.querySelector('h2')).fontSize)}:null,children:[...el.children].map(e=>({tag:e.tagName,cls:e.className,box:box(e)})),rows:[...el.querySelectorAll(':scope > .wp-block-columns')].map(r=>({box:box(r),cols:[...r.children].map(box)}))}))};
 });
 const failures=[];
 ...
 for(const s of metrics.sections){
  ...
  for(const row of s.rows){if(width>=1024&&row.box.w<s.box.w-2*s.pad-3)failures.push(name+': WordPress constrains row width');if(width<640&&row.cols.some(c=>Math.abs(c.w-row.box.w)>2))failures.push(name+': mobile cards not one column');}
 }
 ...
 results.viewports.push({width,reference:ref,metrics,failures});results.failures.push(...failures.map(f=>width+': '+f));console.log(JSON.stringify({width,mode,failures}));await page.close();
}
await fs.writeFile(dir+'/result.json',JSON.stringify(results,null,2));await browser.close();if(results.failures.length)process.exitCode=1;
