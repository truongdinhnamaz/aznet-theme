...
const baseUrl = (process.env.F_HOME_BASE_URL || 'http://127.0.0.1:8080').replace(/\/$/, '');
const outputDir = process.env.F_HOME_STATE_DIR || '/tmp/f-home-native';
...
async function inspectViewport(browser, name, viewport) {
  ...
  try {
    ...
    await page.getByText('F native homepage sentinel', { exact: false }).first().waitFor({ state: 'visible' });
    ...
    console.log(`PASS: f-homepage-native-${name}`);
  } catch (error) {
    result.error = error instanceof Error ? error.message : String(error);
    failures.push(`${name}: ${result.error}`);
    console.error(`FAIL: f-homepage-native-${name}: ${result.error}`);
...
if (failures.length > 0) throw new Error(`F native Homepage L4 verification failed:\n${failures.join('\n')}`);

console.log('PASS: F native Homepage browser/visual/a11y automated verification 3/3');
