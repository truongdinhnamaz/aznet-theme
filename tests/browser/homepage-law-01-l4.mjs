...
async function inspectViewport(browser, name, viewport) {
  ...
  try {
    ...
    if (viewport.width >= 1024) {
      if (!parityMetrics.heroSection || !parityMetrics.heroGrid || !parityMetrics.trustGrid || !parityMetrics.servicesContainer) {
        throw new Error('Hero/shared-shell geometry unavailable');
      }
      ...
      for (const [label, rect] of [['Trust', parityMetrics.trustGrid], ['Services', parityMetrics.servicesContainer]]) {
        if (Math.abs(parityMetrics.heroGrid.x - rect.x) > 2 || Math.abs(parityMetrics.heroGrid.width - rect.width) > 2) {
          throw new Error(`desktop Hero inner grid must align with the common content shell used by ${label}: hero x=${parityMetrics.heroGrid.x.toFixed(1)} width=${parityMetrics.heroGrid.width.toFixed(1)}; ${label.toLowerCase()} x=${rect.x.toFixed(1)} width=${rect.width.toFixed(1)}`);
        }
      }
