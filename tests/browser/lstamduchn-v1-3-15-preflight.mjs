...
const stateDir = process.env.LSTAMDUCHN_STATE_DIR || '/tmp/lstamduchn-v1-3-15-preflight';

fs.mkdirSync(stateDir, { recursive: true });

const state = {
  scope: 'owner-approved-v1.3.15-production-preflight-readonly',
  target: baseUrl,
  authenticated: false,
  support_snapshot_available: false,
  theme: null,
...
async function supportSnapshot(page) {
  ...
  const parsed = JSON.parse(await box.inputValue());
  state.support_snapshot_available = true;
  return parsed;
}
