...
const baseUrl = (process.env.U0_FINAL_BASE_URL || 'http://127.0.0.1:8080').replace(/\/$/, '');
const outputDir = process.env.U0_FINAL_STATE_DIR || '/tmp/u0-final';
...
try {
  for (const [name, viewport] of Object.entries(viewports)) {
    ...
    try {
      ...
      const axeResults = await new AxeBuilder({ page }).include('.aznet-theme-control-center').analyze();
      ...
      await page.screenshot({ path: path.join(screenshotDir, `control-center-${viewport.width}x${viewport.height}.png`), fullPage: true });

      const logoLink = page.getByRole('link', { name: 'Thiết lập Logo' });
      await logoLink.waitFor({ state: 'visible', timeout: 10000 });
      await Promise.all([
        page.waitForURL((url) => url.pathname.endsWith('/wp-admin/customize.php'), { timeout: 15000 }),
        logoLink.click(),
      ]);
      await page.locator('#customize-control-custom_logo').waitFor({ state: 'visible', timeout: 15000 });
      result.customLogoControl = true;

      result.status = 'passed';
      console.log(`PASS: ${name} final Control Center browser/a11y/custom-logo`);
    } catch (error) {
...
if (failures.length > 0) throw new Error(`U0 final browser verification failed:\n${failures.join('\n')}`);
console.log('PASS: U0 final Control Center browser/visual/a11y/custom-logo 2/2');
