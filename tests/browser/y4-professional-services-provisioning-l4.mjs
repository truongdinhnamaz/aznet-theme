...
try {
  ...
  // WordPress owns Page routing. Read the actual View URLs generated by WordPress
  // instead of guessing slugs/permalink structure in Theme QA.
  await page.goto(`${baseUrl}/wp-admin/edit.php?post_type=page`, { waitUntil: 'networkidle' });
  const publicUrls = { home: `${baseUrl}/` };
  for (const [role, title] of [
    ...
    const row = page.locator('tr.type-page').filter({
      has: page.locator('.row-title', { hasText: title }),
    }).first();
    if (await row.count() !== 1) throw new Error(`WordPress Pages list missing fixture row: ${title}`);
    const href = await row.locator('.row-actions .view a').getAttribute('href');
    if (!href) throw new Error(`WordPress did not expose a View URL for fixture Page: ${title}`);
    publicUrls[role] = new URL(href, baseUrl).toString();
  }

  if (consoleErrors.length > 0 || pageErrors.length > 0) throw new Error(`admin browser errors: ${[...consoleErrors, ...pageErrors].join(' | ')}`);
  summary.admin = { recommendationCards: cards, confirmationRequired: true, launchReadyLabel: false, axeBlocking: 0, publicUrls };
  await adminContext.close();

  const routes = [
    ['home', publicUrls.home, 'Giúp khách hàng hiểu vấn đề và chọn bước tiếp theo phù hợp'],
    ['about', publicUrls.about, 'Giới thiệu'],
    ['services', publicUrls.services, 'Dịch vụ'],
    ['team', publicUrls.team, 'Đội ngũ'],
    ['contact', publicUrls.contact, 'Liên hệ'],
  ];
  const viewports = {
  ...
  for (const [viewportName, viewport] of Object.entries(viewports)) {
    ...
    for (const [role, url, heading] of routes) {
      ...
      summary.public.push({ role, url, viewport: viewportName, status: response.status(), h1: heading, overflow, axeBlocking: 0 });
    }
...
console.log('PASS: Y4 Professional Services confirmed wizard + five WordPress-owned public Page surfaces L4');
