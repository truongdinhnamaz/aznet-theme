<?php
/**
 * Y5 integrated client-delivery pre-promotion release contract.
 *
 * @package AZnetTheme
 */
...
$must( 1 === preg_match( '/^Version:\s*1\.2\.0\s*$/m', $style ), 'Y5 pre-promotion style.css must remain exactly 1.2.0' );
$must( str_contains( $functions, "define( 'AZNET_THEME_VERSION', '1.2.0' );" ), 'Y5 pre-promotion AZNET_THEME_VERSION must remain exactly 1.2.0' );
...
foreach ( [
    ...
    $must( str_contains( $workflow, $needle ), 'Y5 workflow missing required pre-promotion marker: ' . $needle );
}
...
echo "PASS: Y5 integrated client-delivery pre-promotion release contract\n";
